...
Real input files next to the SVG pages `docsvg` made from them, so you can
judge the results before installing anything. Every file here is generated by
this repository's own scripts and may be reused freely.

[日本語](README.ja.md) · [Browse them as a gallery](https://ryusui-hiro.github.io/document-svg/samples.html)

To try one yourself after installing (the output folder must be new or empty):

```bash
docsvg samples/source/sample.pptx --output out/slides
```

Each output folder holds `page-0001.svg`, `page-0002.svg`, … and a
`conversion.json` report listing anything that was approximated.
...
- **draw.io** — 3 pages. The first two: shapes and fills, shadows, orthogonal and curved
  connectors with arrowheads, edge labels, and swimlanes with nested children.
  The third shows shapes drawn from a shape library (see below).
